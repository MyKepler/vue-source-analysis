 ...
 * 顺序：props、methods、data、computed、watch，props、methods、data、computed 对象中的属性不能出现重复，优先级和列出顺序一致。
 */
export function initState (vm: Component) {
  vm._watchers = []
  const opts = vm.$options
  // 处理 props 对象，为 props 对象的每个属性设置响应式
  // 并将其代理到 vm 实例上，允许this.propKey的方式访问
  if (opts.props) initProps(vm, opts.props)
  // 处理 methos 对象，校验每个属性的值是否为函数、和 props 属性比对进行判重处理，
  // 并将其代理到 vm 实例上，允许this.methodKey的方式访问，最后得到 vm[key] = methods[key]
   ...
   *   3、判重，computed 中的 key 不能和 data、props、methods 中的属性重复
   */
 ...
 * @param {*} vm 
 * @param {Object} propsOptions num: {default: 3, type: ƒ}
 */
function initProps (vm: Component, propsOptions: Object) {
  // 获取props传给子组件的对象  例如{num:5}
  const propsData = vm.$options.propsData || {}
  const props = vm._props = {}
  // cache prop keys so that future props updates can iterate using Array
  // instead of dynamic object key enumeration.
  // 缓存 props 的每个 key，性能优化 例如["num"]
  const keys = vm.$options._propKeys = []
  // 子组件才有$parent，根组件的$parent为undefined
  ...
  for (const key in propsOptions) {
    ...
    // instantiation here.
    if (!(key in vm)) {
      // 代理 key 到 vm 对象上，允许this.propKey的方式访问
      proxy(vm, `_props`, key)
    }
  }
...
function initData (vm: Component) {
  ...
  while (i--) {
    ...
    if (props && hasOwn(props, key)) {
      ...
      // 代理 data 对象上的属性到 vm 实例，支持通过this.key的方式访问
      proxy(vm, `_data`, key)
    }
  }
  // 设置响应式
...
function initComputed (vm: Component, computed: Object) {
  ...
  for (const key in computed) {
    ...
    if (!(key in vm)) {
      ...
      // 非生产环境有一个判重处理，computed 对象中的属性不能和 data、props、methods 中的属性相同
      if (key in vm.$data) {
...
export function defineComputed (
  ...
  // 拦截对 target.key 的访问和设置，代理到vue实例，允许this.computedKey的形式访问
  Object.defineProperty(target, key, sharedPropertyDefinition)
}
...
function initMethods (vm: Component, methods: Object) {
  ...
  for (const key in methods) {
    ...
    // 将methods中的所有方法赋值到vue实例上，允许this.methodKey的方式访问
...
export function stateMixin (Vue: Class<Component>) {
  ...
  Vue.prototype.$watch = function (
    ...
    // 兼容性处理，因为用户调用 vm.$watch 时设置的 cb 可能是对象，因为可以通过this.$watch()传进来一个对象
    if (isPlainObject(cb)) {
